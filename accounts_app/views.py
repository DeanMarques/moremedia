...
def signup(request):
    if request.method == 'POST':
        if request.POST['password1'] == request.POST['password2']:
            ...
            return render(request, 'accounts_app/signup.html', {'error':'Passwords didn\'t match'})
    else:
        return render(request, 'accounts_app/signup.html')
...
def loginview(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            return render(request, 'accounts_app/login.html', {'error':'The Username and Password didn\'t match'})
    else:
